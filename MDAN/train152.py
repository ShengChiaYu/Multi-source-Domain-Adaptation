	...
	for ep in range(opt.num_epochs):
		...
		for (x,y) in tqdm(data_loader):
			...
			if b%interval==0:
				...
				print("Batch {}, loss = {},accuracy = {}".format(b, b_loss/interval,pred_acc))
				b_loss=0
				model.train()

			
		print("Iteration {}, loss = {}".format(ep, running_loss/b))

		torch.save(model.state_dict(), os.path.join(opt.model_path,'res152_MDAN-{}-epoch-{}.pth'.format(D_name[Dt],ep+1)))
	time_end = time.time()
	torch.save(model.state_dict(), os.path.join(opt.model_path,'res152_MDAN-{}-epoch-{}.pth'.format(D_name[Dt],ep+1)))
...
def main(opt):
	...
	# resnet=model.CNN
	# mdan = MDANet(resnet).to(device)
	# print('loading model from : model/res152_MDAN013-epoch-2.pth')
	# mdan.load_state_dict(torch.load('model/res152_MDAN013-epoch-2.pth'))
