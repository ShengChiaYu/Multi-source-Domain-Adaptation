...
class dataset_public(Dataset):
    def __init__(self, root, transform=None, train=True, domains=None, real_test=False):
        ...
        if self.real_test:
            ...
            if train:
                print('Training data:', domains)
                for domain in domains:
                    domain_root = join(self.root, domain, '{}_train.csv'.format(domain))
                    data = pd.read_csv(domain_root)
                    self.data.append(data)
                self.data = pd.concat(self.data, ignore_index=True)

            else:
                print('Testing data:', domains)
                for domain in domains:
...
def test():
    ...
    dataset = dataset_public(root=dataroot,
                             ...
                             # domains=args.source,
                             # domains=args.target,
                             domains=['infograph', 'sketch', 'quickdraw']
                             # real_test=True,
    )
